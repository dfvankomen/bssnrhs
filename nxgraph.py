...
class ExpressionGraph:
    ...
    def max_storage(self, ordering):
        ...
        # initialize number of dependents for each node
        self.count_dependents()
        current_storage = 0
        max_storage = 0

        dependents_count = {
            node: self._G_.nodes[node]["num_dependents"] for node in self._G_.nodes
        }

        storage_trace = []

        for node in ordering:
            if self._G_.in_degree(node) > 0:
                current_storage += 1

            if current_storage > max_storage:
                max_storage = current_storage

            for dependency in self._G_.successors(node):
                # num = self._G_.nodes[dependency]["num_dependents"]

                num = dependents_count.get(dependency, 0)

                if num > 0:
                    dependents_count[dependency] = num - 1

                    # if it's zero, the life is over
                    if num - 1 == 0:
                        # fully delete dependency
                        current_storage -= 1

                # if num == 0:
                #     # shouldn't happen
                #     raise RuntimeError(
                #         f"Dependency count underflow for node {dependency}"
                #     )
                # elif num == 1:
                #     current_storage -= 1  # delete dependency
                #     self._G_.nodes[dependency]["num_dependents"] = 0
                # else:
                #     self._G_.nodes[dependency]["num_dependents"] = num - 1

            storage_trace.append(current_storage)

        return max_storage, storage_trace

    def random_dfs_sort(self, relevant_nodes=None):
        """
        ...
        # Randomize order of root nodes (no incoming edges)
        roots = [n for n in self._G_.nodes if self._G_.in_degree(n) == 0]
        random.shuffle(roots)

        # make sure the roots have been visited
        for r in roots:
            dfs(r)
